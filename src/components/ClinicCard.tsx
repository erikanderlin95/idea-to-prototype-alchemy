...
import { MapPin, Clock, Users, Star, CheckCircle, Phone, AlertTriangle, Calendar, Shield, MessageCircle, ExternalLink } from "lucide-react";
...
export const ClinicCard = ({
  ...
  const isHarmony = name === "Harmony TCM Centre";
  // Standardized post-join experience across all queue clinics (Harmony TCM format)
  const isNotificationMode = true;
  ...
  const performCancelQueue = async () => {
    ...
    try {
      ...
      if (id) { localStorage.removeItem(`queue_mobile_${id}`); localStorage.removeItem(`queue_nric_${id}`); }
      setMyQueueEntry(null);
    } catch (error: any) {
      ...
      <Card className="group flex flex-col px-3 py-2.5 sm:px-3.5 sm:py-3 hover:shadow-lg transition-all duration-300 border border-emerald-300/70 hover:border-emerald-400 cursor-pointer bg-gradient-to-br from-card to-primary/5 onboarding-join-queue w-full max-w-[360px] md:max-w-[420px] mx-auto h-full" onClick={() => id && navigate(`/clinic/${id}`)}>
        ...
        {myQueueEntry ? (
          ...
          <div className="flex flex-col justify-between gap-1.5 flex-1">
            ...
            {hasDigitalQueue && (
              ...
              >
                <div className="flex items-center gap-2">
                  ...
                  <p className="text-[15px] sm:text-base font-bold text-foreground leading-tight">{isHarmony ? t("clinicCard.lowWait") : t("clinicCard.joinVirtual").replace("{count}", String(queueCount))}</p>
                </div>
            ...
            {(() => {
              const isOtherTCM = type === "TCM" && name !== "Harmony TCM Centre";
              const showBookingButtons = !hasDigitalQueue || name === "Harmony TCM Centre" || isOtherTCM;
              const stackVertical = false;

              const joinQueueBtn = hasDigitalQueue && !isOtherTCM && (
                <Button
